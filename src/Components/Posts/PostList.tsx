...
function PostList() {
  ...
  useEffect(() => {
    const loadPosts = async () => {
      try {
        ...
        // Cache-dan olish
        const cachedPosts = getCachedData<Post[]>(CACHE_KEYS.POSTS);
        console.log("🔍 Checking cache for posts:", !!cachedPosts);

        if (cachedPosts) {
          console.log("📦 Posts loaded from local storage");

          const sortedCachedPosts = cachedPosts.sort(
            (a: Post, b: Post) =>
        ...
        // Agar cache bo‘lmasa, serverdan olish
  ...
  const canScrollRight =
    scrollPosition < posts.length * 320 - 1200;
